...
def receive_alert_data( target_latitude, target_longitude, radius):
...
def receive_curr_data( target_latitude, target_longitude,  radius):
    ...
    if float(current_latitude) == float(target_latitude) and float(current_longitude) == float(target_longitude):
        response_data = {
            'message': 'Alert: Current location is equal to the target location.',
            'alert': True
        }
